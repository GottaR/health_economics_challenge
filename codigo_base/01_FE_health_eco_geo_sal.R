...
AgregarVariables <- function(dataset) {
  ...
  # ============================================================================
  # NUEVAS VARIABLES BASADAS EN IMPORTANCIA
  # ============================================================================
  
  # --- GRUPO 1: RATIOS DE DESARROLLO ECONÓMICO ---
  # Las 3 variables más importantes son económicas: ruralidad, GDP per capita, consumo privado
  
  # Ratio urbanización/desarrollo (invierte la ruralidad)
  dataset[, urban_development_ratio := (100 - SP.RUR.TOTL.ZS) / (NY.GDP.PCAP.KD + 1)]
  
  # Capacidad de consumo privado relativa al GDP
  dataset[, private_consumption_capacity := NE.CON.PRVT.PC.KD / (NY.GDP.PCAP.KD + 1)]
  
  # Proporción del gasto en consumo vs ingreso nacional
  dataset[, consumption_vs_income := NE.CON.PRVT.PC.KD / (NY.GNP.PCAP.CD + 1)]
  
  # --- GRUPO 2: INDICADORES DE PRESIÓN DEMOGRÁFICA ---
  # Población joven (0-24) puede indicar mayor necesidad de gastos de bolsillo
  
  # Carga demográfica joven (necesita más atención médica básica)
  dataset[, young_population_pressure := 
            ifelse(!is.na(SP.POP.2024.FE.5Y) & !is.na(SP.POP.0004.FE.5Y),
                   SP.POP.2024.FE.5Y + SP.POP.0004.FE.5Y,
                   NA_real_)]
  
  # Ratio población urbana vs total (ciudades = más acceso a salud privada)
  dataset[, urban_health_access := SP.URB.TOTL.IN.ZS / 100]
  
  # --- GRUPO 3: EFICIENCIA DEL GASTO EN SALUD ---
  # Mortalidad infantil es indicador clave de eficiencia del sistema
  
  # Mortalidad ajustada por GDP (países ricos deberían tener menos mortalidad)
  dataset[, mortality_efficiency := 
            SH.DTH.MORT / (NY.GDP.PCAP.KD / 1000 + 1)]
  
  # Mortalidad neonatal como señal de sistema de salud débil
  dataset[, neonatal_risk_indicator := 
            ifelse(!is.na(SH.DTH.NMRT), 
                   SH.DTH.NMRT / (SH.DTH.MORT + 1),
                   NA_real_)]
