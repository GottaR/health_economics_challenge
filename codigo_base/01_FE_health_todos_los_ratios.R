...
AgregarVariables <- function(dataset) {
  ...
  cat("\n--- Creando Ratios Económicos ---\n")
  
  # 1.1 Gasto en Salud / PIB
  if(all(c("SH.XPD.CHEX.PC.CD", "NY.GDP.PCAP.CD") %in% names(dataset))) {
    dataset[, health_spending_gdp_ratio := SH.XPD.CHEX.PC.CD / (NY.GDP.PCAP.CD + 1)]
    cat("✓ health_spending_gdp_ratio\n")
  }
  
  # 1.2 Gasto Público en Salud / Gasto Total en Salud
  if(all(c("SH.XPD.GHED.PC.CD", "SH.XPD.CHEX.PC.CD") %in% names(dataset))) {
    dataset[, public_health_share := SH.XPD.GHED.PC.CD / (SH.XPD.CHEX.PC.CD + 1)]
    cat("✓ public_health_share\n")
  }
  
  # 1.3 Consumo Privado / PIB
  if(all(c("NE.CON.PRVT.CD", "NY.GDP.MKTP.CD") %in% names(dataset))) {
    dataset[, private_consumption_gdp := NE.CON.PRVT.CD / (NY.GDP.MKTP.CD + 1)]
    cat("✓ private_consumption_gdp\n")
  }
  
  # 1.4 Consumo Gobierno / PIB
  if(all(c("NE.CON.GOVT.CD", "NY.GDP.MKTP.CD") %in% names(dataset))) {
    dataset[, government_consumption_gdp := NE.CON.GOVT.CD / (NY.GDP.MKTP.CD + 1)]
    cat("✓ government_consumption_gdp\n")
  }
  
  # 1.5 Apertura Comercial (Exportaciones + Importaciones) / PIB
  if(all(c("NE.EXP.GNFS.CD", "NE.IMP.GNFS.CD", "NY.GDP.MKTP.CD") %in% names(dataset))) {
    dataset[, trade_openness := (NE.EXP.GNFS.CD + NE.IMP.GNFS.CD) / (NY.GDP.MKTP.CD + 1)]
    cat("✓ trade_openness\n")
  }
  
  # 1.6 Remesas / PIB
  if(all(c("BX.TRF.PWKR.CD.DT", "NY.GDP.MKTP.CD") %in% names(dataset))) {
    dataset[, remittances_gdp := BX.TRF.PWKR.CD.DT / (NY.GDP.MKTP.CD + 1)]
    cat("✓ remittances_gdp\n")
  }
  
  # 1.7 Gasto Militar / Gasto en Salud (trade-off)
  if(all(c("MS.MIL.XPND.GD.ZS", "SH.XPD.CHEX.GD.ZS") %in% names(dataset))) {
    dataset[, military_health_ratio := MS.MIL.XPND.GD.ZS / (SH.XPD.CHEX.GD.ZS + 0.1)]
    cat("✓ military_health_ratio\n")
  }
  
  # ========================================
  # SECCIÓN 2: ÍNDICES DE SALUD COMPUESTOS
  # ========================================
  
  cat("\n--- Creando Índices de Salud ---\n")
  
  # 2.1 Índice de Salud Materno-Infantil
  if(all(c("SH.DTH.IMRT", "SH.STA.MMRT", "SH.DTH.MORT") %in% names(dataset))) {
    dataset[, maternal_child_health_index := 
              (SH.DTH.IMRT + SH.STA.MMRT/10 + SH.DTH.MORT) / 3]
    cat("✓ maternal_child_health_index\n")
  }
  
  # 2.2 Índice de Mortalidad Adulta (promedio hombres y mujeres)
  if(all(c("SH.DYN.MORT.FE", "SH.DYN.MORT.MA") %in% names(dataset))) {
    dataset[, adult_mortality_index := (SH.DYN.MORT.FE + SH.DYN.MORT.MA) / 2]
    cat("✓ adult_mortality_index\n")
  }
  
  # 2.3 Brecha de Género en Mortalidad Adulta
  if(all(c("SH.DYN.MORT.MA", "SH.DYN.MORT.FE") %in% names(dataset))) {
    dataset[, mortality_gender_gap := SH.DYN.MORT.MA - SH.DYN.MORT.FE]
    cat("✓ mortality_gender_gap\n")
  }
  
  # 2.4 Cobertura de Inmunización Promedio
  if(all(c("SH.IMM.IDPT", "SH.IMM.MEAS") %in% names(dataset))) {
    dataset[, immunization_coverage := (SH.IMM.IDPT + SH.IMM.MEAS) / 2]
    cat("✓ immunization_coverage\n")
  }
  
  # 2.5 Índice de Sistema de Salud (inverso de mortalidad + expectativa vida)
  if(all(c("SP.DYN.LE00.IN", "SH.DTH.IMRT") %in% names(dataset))) {
    dataset[, health_system_index := SP.DYN.LE00.IN / (SH.DTH.IMRT + 1)]
    cat("✓ health_system_index\n")
  }
  
  # ========================================
  # SECCIÓN 3: VARIABLES DEMOGRÁFICAS
  # ========================================
  
  cat("\n--- Creando Variables Demográficas ---\n")
  
  # 3.1 Índice de Envejecimiento (65+ / 0-14)
  if(all(c("SP.POP.65UP.TO.ZS", "SP.POP.0014.TO.ZS") %in% names(dataset))) {
    dataset[, aging_index := SP.POP.65UP.TO.ZS / (SP.POP.0014.TO.ZS + 1)]
    cat("✓ aging_index\n")
  }
  
  # 3.2 Proporción de Población Económicamente Activa
  if(all(c("SP.POP.1564.TO.ZS", "SP.POP.TOTL") %in% names(dataset))) {
    dataset[, working_age_proportion := SP.POP.1564.TO.ZS / 100]
    cat("✓ working_age_proportion\n")
  }
  
  # 3.3 Tasa de Urbanización
  if("SP.URB.TOTL.IN.ZS" %in% names(dataset)) {
    dataset[, urbanization_rate := SP.URB.TOTL.IN.ZS / 100]
    cat("✓ urbanization_rate\n")
  }
  
  # 3.4 Cambio en Urbanización (requiere lag, calcularlo aquí)
  if("SP.URB.TOTL.IN.ZS" %in% names(dataset)) {
    dataset[, urbanization_lag1 := shift(SP.URB.TOTL.IN.ZS, 1, NA, "lag"), 
            by = `Country Code`]
    dataset[, urbanization_change := SP.URB.TOTL.IN.ZS - urbanization_lag1]
    cat("✓ urbanization_change\n")
  }
  
  # 3.5 Densidad Poblacional
  if(all(c("SP.POP.TOTL", "AG.LND.TOTL.K2") %in% names(dataset))) {
    dataset[, population_density := SP.POP.TOTL / (AG.LND.TOTL.K2 + 1)]
    cat("✓ population_density\n")
  }
  
  # 3.6 Ratio de Dependencia Total Ajustado
  if("SP.POP.DPND" %in% names(dataset)) {
    dataset[, dependency_ratio_scaled := SP.POP.DPND / 100]
    cat("✓ dependency_ratio_scaled\n")
  }
  
  # ========================================
  # SECCIÓN 4: VARIABLES DE DESARROLLO
  # ========================================
  
  cat("\n--- Creando Variables de Desarrollo ---\n")
  
  # 4.1 Expectativa de Vida al Cuadrado (capturar no-linealidad)
  if("SP.DYN.LE00.IN" %in% names(dataset)) {
    dataset[, life_expectancy_squared := SP.DYN.LE00.IN^2]
    cat("✓ life_expectancy_squared\n")
  }
  
  # 4.2 Log del PIB per cápita
  if("NY.GDP.PCAP.CD" %in% names(dataset)) {
    dataset[, log_gdp_per_capita := log(NY.GDP.PCAP.CD + 1)]
    cat("✓ log_gdp_per_capita\n")
  }
  
  # 4.3 Tasa de Matrícula Secundaria Promedio
  if(all(c("SE.SEC.ENRR.FE", "SE.SEC.ENRR.MA") %in% names(dataset))) {
    dataset[, secondary_enrollment_avg := (SE.SEC.ENRR.FE + SE.SEC.ENRR.MA) / 2]
    cat("✓ secondary_enrollment_avg\n")
  }
  
  # 4.4 Brecha de Género en Educación
  if(all(c("SE.SEC.ENRR.FE", "SE.SEC.ENRR.MA") %in% names(dataset))) {
    dataset[, education_gender_gap := SE.SEC.ENRR.FE - SE.SEC.ENRR.MA]
    cat("✓ education_gender_gap\n")
  }
  
  # 4.5 Acceso a Electricidad per cápita (proxy desarrollo)
  if("EG.USE.ELEC.KH.PC" %in% names(dataset)) {
    dataset[, log_electricity_consumption := log(EG.USE.ELEC.KH.PC + 1)]
    cat("✓ log_electricity_consumption\n")
  }
  
  # ========================================
  # SECCIÓN 5: VARIABLES DE ESTABILIDAD ECONÓMICA
  # ========================================
  
  cat("\n--- Creando Variables de Estabilidad ---\n")
  
  # 5.1 Dummy de Alta Inflación (>10%)
  if("FP.CPI.TOTL.ZG" %in% names(dataset)) {
    dataset[, high_inflation := ifelse(FP.CPI.TOTL.ZG > 10, 1, 0)]
    cat("✓ high_inflation\n")
  }
  
  # 5.2 Dummy de Hiperinflación (>50%)
  if("FP.CPI.TOTL.ZG" %in% names(dataset)) {
    dataset[, hyperinflation := ifelse(FP.CPI.TOTL.ZG > 50, 1, 0)]
    cat("✓ hyperinflation\n")
  }
  
  # 5.3 Inflación al Cuadrado (capturar efectos no-lineales)
  if("FP.CPI.TOTL.ZG" %in% names(dataset)) {
    dataset[, inflation_squared := (FP.CPI.TOTL.ZG)^2]
    cat("✓ inflation_squared\n")
  }
  
  # 5.4 Reservas Internacionales en Meses de Importaciones
  if("FI.RES.TOTL.MO" %in% names(dataset)) {
    dataset[, reserves_adequacy := FI.RES.TOTL.MO]
    cat("✓ reserves_adequacy\n")
  }
  
  # 5.5 Dummy de Crisis de Reservas (<3 meses)
  if("FI.RES.TOTL.MO" %in% names(dataset)) {
    dataset[, reserves_crisis := ifelse(FI.RES.TOTL.MO < 3, 1, 0)]
    cat("✓ reserves_crisis\n")
  }
  
  # 5.6 Profundidad Financiera (Crédito Privado / PIB)
  if("FD.AST.PRVT.GD.ZS" %in% names(dataset)) {
    dataset[, financial_depth := FD.AST.PRVT.GD.ZS / 100]
    cat("✓ financial_depth\n")
  }
  
  # ========================================
  # SECCIÓN 6: VARIABLES DE AGRICULTURA Y AMBIENTE
  # ========================================
  
  cat("\n--- Creando Variables Agrícolas/Ambientales ---\n")
  
  # 6.1 Productividad Agrícola
  if("AG.YLD.CREL.KG" %in% names(dataset)) {
    dataset[, log_cereal_yield := log(AG.YLD.CREL.KG + 1)]
    cat("✓ log_cereal_yield\n")
  }
  
  # 6.2 Tierra Arable per cápita
  if(all(c("AG.LND.ARBL.HA.PC", "SP.POP.TOTL") %in% names(dataset))) {
    dataset[, arable_land_pressure := 1 / (AG.LND.ARBL.HA.PC + 0.001)]
    cat("✓ arable_land_pressure\n")
  }
  
  # 6.3 Emisiones CO2 per cápita
  if("EN.ATM.CO2E.PC" %in% names(dataset)) {
    dataset[, log_co2_per_capita := log(EN.ATM.CO2E.PC + 1)]
    cat("✓ log_co2_per_capita\n")
  }
  
  # 6.4 Intensidad de Emisiones (CO2 / PIB)
  if(all(c("EN.ATM.CO2E.KT", "NY.GDP.MKTP.CD") %in% names(dataset))) {
    dataset[, emissions_intensity := EN.ATM.CO2E.KT / (NY.GDP.MKTP.CD + 1)]
    cat("✓ emissions_intensity\n")
  }
  
  # ========================================
  # SECCIÓN 7: DUMMIES DE REGIÓN
  # ========================================
  
  cat("\n--- Creando Dummies de Región ---\n")
  
  if("region" %in% names(dataset)) {
    ...
    dataset[, region_WPR := ifelse(region == "WPR", 1, 0)]
    cat("✓ Dummies de región (6 variables)\n")
  }
  
  # ========================================
  # SECCIÓN 8: DUMMIES DE NIVEL DE INGRESO
  # ========================================
  
  cat("\n--- Creando Dummies de Nivel de Ingreso ---\n")
  ...
  if("income" %in% names(dataset)) {
    ...
    dataset[, income_High := ifelse(income == "High", 1, 0)]
    cat("✓ Dummies de ingreso (4 variables)\n")
  }
  
  # ========================================
  # SECCIÓN 9: INTERACCIONES CLAVE
  # ========================================
  
  cat("\n--- Creando Interacciones ---\n")
  
  # 9.1 Envejecimiento × Gasto en Salud
  if(all(c("SP.POP.65UP.TO.ZS", "SH.XPD.CHEX.GD.ZS") %in% names(dataset))) {
    dataset[, aging_health_spending_interaction := 
              SP.POP.65UP.TO.ZS * SH.XPD.CHEX.GD.ZS]
    cat("✓ aging_health_spending_interaction\n")
  }
  
  # 9.2 Urbanización × PIB per cápita
  if(all(c("SP.URB.TOTL.IN.ZS", "NY.GDP.PCAP.CD") %in% names(dataset))) {
    dataset[, urbanization_gdp_interaction := 
              SP.URB.TOTL.IN.ZS * log(NY.GDP.PCAP.CD + 1)]
    cat("✓ urbanization_gdp_interaction\n")
  }
  
  # 9.3 Inflación × Apertura Comercial
  if(all(c("FP.CPI.TOTL.ZG", "TG.VAL.TOTL.GD.ZS") %in% names(dataset))) {
    dataset[, inflation_trade_interaction := 
              FP.CPI.TOTL.ZG * TG.VAL.TOTL.GD.ZS]
    cat("✓ inflation_trade_interaction\n")
  }
  
  # 9.4 Educación × Expectativa de Vida
  if(all(c("SE.SEC.ENRR", "SP.DYN.LE00.IN") %in% names(dataset))) {
    dataset[, education_life_expectancy_interaction := 
              SE.SEC.ENRR * SP.DYN.LE00.IN]
    cat("✓ education_life_expectancy_interaction\n")
  }
  
  # ========================================
  # SECCIÓN 10: DUMMIES TEMPORALES
  # ========================================
  
  cat("\n--- Creando Dummies Temporales ---\n")
  
  # 10.1 Dummy Crisis Financiera 2008-2009
  if("year" %in% names(dataset)) {
    dataset[, crisis_2008 := ifelse(year %in% c(2008, 2009), 1, 0)]
    cat("✓ crisis_2008\n")
  }
  
  # 10.2 Dummy COVID-19 2020-2021
  if("year" %in% names(dataset)) {
    dataset[, covid_period := ifelse(year %in% c(2020, 2021), 1, 0)]
    cat("✓ covid_period\n")
  }
  
  # 10.3 Dummy Post-2010 (época moderna)
  if("year" %in% names(dataset)) {
    dataset[, post_2010 := ifelse(year >= 2010, 1, 0)]
    cat("✓ post_2010\n")
  }
  
  # 10.4 Dummy Década
  if("year" %in% names(dataset)) {
    dataset[, decade_2000s := ifelse(year >= 2000 & year < 2010, 1, 0)]
    dataset[, decade_2010s := ifelse(year >= 2010 & year < 2020, 1, 0)]
    dataset[, decade_2020s := ifelse(year >= 2020, 1, 0)]
    cat("✓ Dummies de década (3 variables)\n")
  }
  
  # ========================================
  # SECCIÓN 11: INTERACCIONES COVID × REGIÓN
  # ========================================
  
  cat("\n--- Creando Interacciones COVID × Región ---\n")
  
  if(all(c("year", "region") %in% names(dataset))) {
    dataset[, covid_AFR := ifelse(year %in% c(2020, 2021) & region == "AFR", 1, 0)]
    dataset[, covid_AMR := ifelse(year %in% c(2020, 2021) & region == "AMR", 1, 0)]
    dataset[, covid_EUR := ifelse(year %in% c(2020, 2021) & region == "EUR", 1, 0)]
    cat("✓ Interacciones COVID × Región (3 principales)\n")
  }
