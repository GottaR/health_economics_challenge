...
AgregarVariables <- function(dataset) {
  ...
  # R4: Balance comercial (exportaciones vs importaciones)
  # Valores de mercancías: mide dependencia externa
  dataset[, trade_balance_ratio := 
            TX.VAL.MRCH.R2.ZS / (TM.VAL.MRCH.R2.ZS + 1)]
  
  # R5: Presión demográfica ajustada por desarrollo
  # Carga de dependencia ponderada por capacidad económica
  dataset[, demographic_pressure := 
            SP.POP.DPND / (NY.GDP.PCAP.KD / 1000 + 1)]

  
  
  # === CRISIS Y CONTEXTO (0 colinealidad con features continuas) ===
  
  # Crisis Financiera Global
  dataset[, crisis_2008 := as.integer(year %in% c(2008, 2009))]
  dataset[, post_crisis := as.integer(year %in% c(2010, 2011, 2012))]
  
  # COVID-19
  dataset[, pre_covid := as.integer(year == 2019)]
  dataset[, covid_year := as.integer(year == 2020)]
  
  # Recesión económica (PIB negativo)
  dataset[, in_recession := as.integer(!is.na(NY.GDP.PCAP.KD.ZG) & 
                                         NY.GDP.PCAP.KD.ZG < 0)]
  
  # Años de alta inflación
  dataset[, high_inflation := as.integer(!is.na(FP.CPI.TOTL.ZG) & 
                                           FP.CPI.TOTL.ZG > 10)]
  
  # Interacciones crisis × desarrollo
  dataset[, crisis_impact_rich := crisis_2008 * 
            as.integer(!is.na(NY.GDP.PCAP.KD) & NY.GDP.PCAP.KD > 10000)]
  
  dataset[, crisis_impact_poor := crisis_2008 * 
            as.integer(!is.na(NY.GDP.PCAP.KD) & NY.GDP.PCAP.KD < 5000)]
