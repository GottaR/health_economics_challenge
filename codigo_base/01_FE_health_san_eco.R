...
AgregarVariables <- function(dataset) {
  ...
  # }
  # R1: Eficiencia del consumo privado (poder adquisitivo real)
  # Mide cuánto consume per cápita ajustado por tamaño económico
  dataset[, consumption_efficiency := 
            NE.CON.PRVT.PC.KD / (NY.GDP.PCAP.KD + 1)]
  
  # R2: Retorno en longevidad por desarrollo económico
  # Años de vida esperada por cada $1,000 de PIB per cápita
  dataset[, longevity_per_gdp := 
            SP.DYN.LE00.IN / (NY.GDP.PCAP.KD / 1000 + 1)]
  
  # R3: Cobertura sanitaria ajustada por desarrollo
  # Eficiencia del sistema: % inmunización / riqueza del país
  dataset[, health_coverage_efficiency := 
            SH.IMM.IDPT / (NY.GDP.PCAP.KD / 1000 + 1)]
